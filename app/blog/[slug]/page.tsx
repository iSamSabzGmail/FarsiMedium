...
// ۱. تعریف دقیق ساختار داده‌ها (برای رفع ارور تایپ‌اسکریپت)
interface Article {
  id: string;
  title: string;
  summary: string;
...
// ۲. تابع ساخت صفحات استاتیک (ضد ضربه برای بیلد گیت‌هاب)
export async function generateStaticParams() {
  try {
    const { data: articles } = await supabase.from('articles').select('slug, id');
    
    // اگر دیتابیس خالی بود یا ارور داد، آرایه خالی برمی‌گرداند تا بیلد متوقف نشود
    if (!articles) return [];
    ...
    console.warn('⚠️ هشدار بیلد: اتصال به دیتابیس برای ساخت صفحات استاتیک برقرار نشد (این طبیعی است).');
    return [];
  }
}

// ۳. تابع دریافت اطلاعات مقاله و مقالات مرتبط
async function getArticleData(slug: string) {
  // الف) دریافت مقاله اصلی
  ...
  if (article) {
    const { data } = await supabase
      ...
      .neq('id', article.id) // خود مقاله فعلی را نیار
      .limit(3);
    ...
    <div className="min-h-screen text-gray-200 font-vazir pb-20" dir="rtl">
      ...
      <div className="sticky top-0 z-50 bg-[#050505]/60 backdrop-blur-xl border-b border-white/5 px-6 h-16 flex items-center justify-between supports-[backdrop-filter]:bg-[#050505]/60">
        ...
        <div className="flex gap-2 relative">
           <Link href="/" className="p-2.5 hover:bg-white/10 rounded-full text-gray-400 hover:text-white transition-colors"><Home size={20} /></Link>
           <div className="relative">
            <button onClick={() => setShowShareMenu(!showShareMenu)} className={`p-2.5 rounded-full transition-colors ${showShareMenu ? 'bg-blue-600 text-white' : 'hover:bg-white/10 text-gray-400'}`}><Share2 size={20} /></button>
            <AnimatePresence>
              {showShareMenu && (
