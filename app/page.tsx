...
import { Search, Clock, ArrowLeft, Zap, Layers, Send, CheckCircle2 } from 'lucide-react';
import Link from 'next/link';
import Navbar from '@/components/Navbar';

// لیست عکس‌های رندوم و باکیفیت (Fallback)
const FALLBACK_IMAGES = [
  ...
  "https://images.unsplash.com/photo-1519389950473-47ba0277781c?auto=format&fit=crop&w=800&q=80", // تیم
  "https://images.unsplash.com/photo-1451187580459-43490279c0fa?auto=format&fit=crop&w=800&q=80", // شبکه
];
...
export default function Home() {
  ...
  // تابع کمکی برای انتخاب عکس
  const getCoverImage = (url: string | null, index: number) => {
      // اگر لینک وجود داشت و با http شروع میشد، خودش را برگردان
      if (url && url.startsWith('http') && url.length > 10) return url;
      // در غیر این صورت، یک عکس رندوم از لیست برگردان
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 pt-32 pb-20 relative z-10">
        ...
        <div className="text-center max-w-5xl mx-auto mb-20">
          ...
          <div className="relative max-w-2xl mx-auto group">
            ...
            <div className="relative flex items-center glass rounded-2xl px-4 py-3 focus-within:border-green-500/50 transition-all shadow-2xl">
              ...
              <input type="text" placeholder="جستجو کنید..." className="bg-transparent border-none outline-none text-white w-full h-full px-2 placeholder-gray-500 text-lg font-medium" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
            </div>
          </div>
        </div>
        ...
        <div className="mb-12 overflow-x-auto pb-4 scrollbar-hide">
            <div className="flex justify-center gap-3 min-w-max px-4">
                {categories.map(cat => (
                    <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-6 py-2.5 rounded-full text-sm font-bold transition-all border whitespace-nowrap ${selectedCategory === cat ? 'bg-green-600 text-white border-green-500 shadow-[0_0_20px_-5px_rgba(34,197,94,0.6)]' : 'glass text-gray-400 border-white/5 hover:text-white hover:border-white/20'}`}>
                        {cat}
                    </button>
                ))}
            </div>
        ...
        {filtered.length > 0 ? (
            ...
            {filtered.map((article, index) => (
                ...
                <div className="aspect-[16/10] relative overflow-hidden m-2 rounded-[2rem] bg-[#111]">
                    <img 
                        src={getCoverImage(article.cover_url, index)} 
                        alt={article.title} 
                        className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700"
                        onError={(e) => { e.currentTarget.src = FALLBACK_IMAGES[index % FALLBACK_IMAGES.length]; }} 
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-60" />
                    <span className="absolute top-4 right-4 bg-black/40 backdrop-blur-md px-3 py-1.5 rounded-xl text-[11px] font-bold text-white border border-white/10 shadow-lg">{article.category}</span>
                </div>

                <div className="p-6 flex flex-col flex-1">
                    <h3 className="text-xl font-bold mb-4 text-white group-hover:text-green-400 transition-colors line-clamp-2 leading-snug">{article.title}</h3>
                    <p className="text-gray-400 text-sm line-clamp-2 mb-6 leading-7 flex-1 font-light">{article.summary}</p>
                    ...
                    <span className="text-white flex items-center gap-1 bg-white/5 px-4 py-2 rounded-lg group-hover:bg-green-600 group-hover:text-white transition-all font-bold">بخوانید <ArrowLeft size={14}/></span>
                    </div>
                </div>
                </Link>
            ))}
        ...
        <div className="relative max-w-4xl mx-auto">
           ...
           <div className="relative glass rounded-[3rem] p-8 md:p-16 text-center border border-white/10 overflow-hidden">
             ...
             <div className="flex flex-col sm:flex-row gap-4 relative z-10">
               <input type="text" placeholder="https://medium.com/..." className="flex-1 bg-black/40 border border-white/10 rounded-2xl px-6 py-4 text-left dir-ltr focus:border-green-500 outline-none transition-colors text-white placeholder-gray-500" value={requestUrl} onChange={(e) => setRequestUrl(e.target.value)} />
               <button onClick={submitRequest} disabled={requestStatus !== 'idle'} className={`px-10 py-4 rounded-2xl font-bold transition-all flex items-center justify-center gap-2 shadow-xl ${requestStatus === 'success' ? 'bg-green-600 text-white' : 'bg-white text-black hover:bg-green-400 hover:scale-105'}`}>{requestStatus === 'loading' ? '...' : requestStatus === 'success' ? <CheckCircle2/> : <><Send size={20}/> ارسال</>}</button>
             </div>
           </div>
        </div>
